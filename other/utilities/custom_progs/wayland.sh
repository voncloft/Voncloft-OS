...
###show url in tags
grep -i "href" $file | grep .tar.xz | grep -Po '(?<=href=")[^"]*' | egrep -o "([0-9]{1,}\.)+[0-9]{1,}" > test.txt
...
#sed -i -e '/-v1/d' test.txt
#sed -i -e "/$name/d" test.txt
#sed -i -e "s/v//g" test.txt
#sed -i -e 's/v//g' test.txt

###beta###
command="cat test.txt | sort -V -r | head -n1"
eval $command
size=$(eval $command)
