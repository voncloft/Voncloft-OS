...
grep -e "tar.xz" index.html | grep -Po "(?<=>)[^<>]*(?=<)" | grep -v sig | grep "tar.xz" | sed "s/-$latest/\n$latest/g" | sed 's/.tar.xz//g'
echo "weeeee"
echo "$url${latest}portingAids/"
echo "https://download.kde.org/stable/frameworks/5.81/portingAids/"
